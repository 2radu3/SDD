...
//stiva - LIFO
//coada - FIFO
//TEMA!!!!!!!!!!! inserat la sfarsit si extractat de la sfarsit
//unde inserez de acolo fac si extragerea
//inserare la inceput si extragere la final TEMA!!!!!!!!!
...
void conversieCoadaLaVector(Nod** coada, Carte** vectorCarti, int nrCarti) {
...
int main() {
	...
	//printf("\nTraversare coada: ");
	//while (coada != NULL) {
	//	c = pop(&coada); //pop este get
	//	afisareCarte(c);
	//	free(c.titlu);
	//}
//COADA FIFO STIVA LIFO LA TEST!!
	Carte* vectorCarti = NULL;
	int nrCarti = 4;
	conversieCoadaLaVector(&coada, &vectorCarti, nrCarti);
	printf("\nTraversareVector: ");
	if (vectorCarti != NULL) {
		for (int i = 0; i < nrCarti; i++) {
